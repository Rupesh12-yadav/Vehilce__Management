  ...
  const fetchReports = async () => {
    try {
      ...
      const [statsRes, usersRes, vehiclesRes, bookingsRes] = await Promise.all([
        API.get('/users/stats').catch(e => { console.error('Stats API error:', e); return { data: { data: {} } }; }),
        API.get('/users').catch(e => { console.error('Users API error:', e); return { data: { data: [] } }; }),
        API.get('/vehicles').catch(e => { console.error('Vehicles API error:', e); return { data: { data: [] } }; }),
        API.get('/bookings').catch(e => { console.error('Bookings API error:', e); return { data: { data: [] } }; }),
      ]);
      ...
      // Users stats - use API stats if available, otherwise calculate
      const usersStats = {
        total: apiStats.totalUsers || users.length,
        drivers: apiStats.drivers || users.filter(u => u.role === 'driver').length,
        admins: apiStats.vehicleAdmins || users.filter(u => u.role === 'vehicleadmin').length,
        active: apiStats.activeUsers || users.filter(u => u.isActive).length,
      };

      // Vehicles stats
      const vehiclesStats = {
        total: apiStats.totalVehicles || vehicles.length,
        available: apiStats.availableVehicles || vehicles.filter(v => v.availability).length,
        booked: (apiStats.totalVehicles || vehicles.length) - (apiStats.availableVehicles || vehicles.filter(v => v.availability).length),
      };

      // Bookings stats
      const bookingsStats = {
        total: apiStats.totalBookings || bookings.length,
        completed: apiStats.completedBookings || bookings.filter(b => b.status === 'completed').length,
        cancelled: bookings.filter(b => b.status === 'cancelled').length,
        pending: apiStats.pendingBookings || bookings.filter(b => b.status === 'pending').length,
      };

      // Earnings stats
      const earningsStats = {
        total: apiStats.totalRevenue || bookings
